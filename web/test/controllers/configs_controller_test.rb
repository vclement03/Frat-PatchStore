...
class ConfigsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create config" do
    assert_difference('Config.count') do
      post configs_url, params: { config: { description: @config.description, key: @config.key, value: @config.value } }
    end
  ...
  test "should update config" do
    patch config_url(@config), params: { config: { description: @config.description, key: @config.key, value: @config.value } }
